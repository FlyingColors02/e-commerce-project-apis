...
let subCategorySchema = new  mongoose.Schema({
    subCategoryName:{type: String, min: 3, max: 300,lowercase:true,trim:true}
});

let subCategoryModel = mongoose.model( "subcategoryrecords", subCategorySchema);

let CategorySchema  = new mongoose.Schema({
    categoryName: { type: String, min: 3, max: 100,required:true,lowercase:true,trim:true},
...
let mainCategorySchema = new mongoose.Schema({
    mainCategoryName: {type: String, min:3, max:50, required: true,lowercase:true,trim:true},
...
let ProductSchema = new mongoose.Schema({
    productName: { type: String, min: 3, max: 300, required: true, lowercase:true},
    ...
    Productcategory: {type: [CategorySchema],ref:"productrecords"},
    ProductsubCategory: { type: [subCategorySchema],ref:"subcategoryrecords"},
